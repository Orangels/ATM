...
__global__ void get3Dkerel(float* vipKeyPoints, float* vopKeyPoints, float* vpBoxes, cudaTextureObject_t w_shp_base, cudaTextureObject_t w_exp_base)
{
	__shared__ float s_src[62];
	int tid = threadIdx.x;
	if (threadIdx.x < 62)
	{
		float src = vipKeyPoints[62 * blockIdx.x+ threadIdx.x];
	...
	int offset = blockIdx.x*219 +  threadIdx.x;
	vopKeyPoints[offset] = temp;
	...
	{
		offset +=  204;
		vopKeyPoints[offset] = s_src[threadIdx.x];
	}
	if (threadIdx.x < 3)
	{
		offset +=  12;
		vopKeyPoints[offset] = s_src[12+threadIdx.x];
	}
	//*/
}
...
{
	get3Dkerel << <vBatchSize, 204, 0, vCudaStream >> > (vikeyPoints, vokeyPoints, vpBoxes, vShpBase,vExpBase);
}