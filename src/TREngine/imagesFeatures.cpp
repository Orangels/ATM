...
void GetPinYin(unsigned char* Chinese, std::string& PinYin);
int Utf8ToGbk(char *src_str, size_t src_len, char *dst_str, size_t dst_len);

void CImagesFeatures::_normalize(float* vp)
{
	double sum = 0;
	float* p = vp;
...
CImagesFeatures::CImagesFeatures() :m_pfeatrues(NULL), m_buffer(NULL)
{
	m_pconfiger = CConfiger::getOrCreateConfiger();
	m_pfeatrues = new CFaceFeatures;
	m_pio = new CProtoIO;
	m_videoOrImgPath = m_pconfiger->readValue("videoNames");
	
	if (isImagesPath())
		readPathOrText(m_videoOrImgPath, m_imgNames, [](const std::string& vfile) {return vfile.length() > 4; }, 0, false);

	m_onlineFlag = isStartWith(m_pconfiger->readValue("recognizationRunModel"), "online");
	m_pinyin = ("pinyin" == m_pconfiger->readValue("personNameType"));
	if (m_onlineFlag)
	{
		//cublasSgemv_v2
		cudaMallocHost((void**)&m_buffer, 512 * sizeof(float));
		auto featuresFileName = m_pconfiger->readValue("faceFeaturesFileName");
		m_pio->readProtoFromBinaryFile(featuresFileName, m_pfeatrues);
		m_personCount = m_pfeatrues->persons_size();
		std::cout << "There are " << m_personCount << " persons in " << featuresFileName << std::endl;
	}
	else
		m_pfeatrues->set_imagepath(m_videoOrImgPath);
	m_pfaceQuery = new CGPUFaceQuery(m_onlineFlag?m_pfeatrues:NULL);
	m_recognizationConfidence = m_pconfiger->readValue<float>("recognizationConfidence");
}

CImagesFeatures::~CImagesFeatures()
{
	delete m_pio;
	delete m_pfeatrues;
	if (m_pfaceQuery) delete m_pfaceQuery;
	if (m_buffer)cudaFreeHost(m_buffer);
}

void CImagesFeatures::addFeature(int vfaceId, float* vfeature, void* vcudaStream)
{
	std::string name = deleteExtentionName(m_imgNames[vfaceId], false);
	std::vector<char> gbkName(name.size() + 1, 0);
	Utf8ToGbk((char*)name.c_str(), name.size(), (char*)gbkName.data(), gbkName.size());

	std::string namePinYin;// = std::to_string(vfaceId);
	GetPinYin((unsigned char*)gbkName.data(), namePinYin);
	CPerson* pperson = m_pfeatrues->add_persons();
	pperson->set_id(vfaceId);	
	pperson->set_name(name+"_"+namePinYin);
	std::cout << name << " add " << namePinYin << std::endl;
// 	_normalize(vfeature);
	m_pfaceQuery->calculateL2Normal(vfeature, vcudaStream);
	std::vector<float> feature(512);
	cudaMemcpyAsync(feature.data(), vfeature, feature.size() * sizeof(feature.front()), cudaMemcpyDeviceToHost, (cudaStream_t)vcudaStream);
	cudaStreamSynchronize((cudaStream_t)vcudaStream);
	vfeature = feature.data();
	for (int i = 0; i < 512; ++i)
		pperson->add_featrue(*vfeature++);
	if (vfaceId + 1 == m_imgNames.size())
	{
		std::string faceFeaturesFileName = m_pconfiger->readValue("faceFeaturesFileName");
		std::cout << "Finish building face feature data set!" << std::endl;
		m_pio->writeProto2BinaryFile(faceFeaturesFileName, m_pfeatrues);
		exit(0);
	}
}

// #include "FileFunction.h"
int CImagesFeatures::whoAmI(float* vpfeature, void* vcudaStream)
{
 	cudaStream_t s = (cudaStream_t)vcudaStream;
// 	size_t bites = 512 * 4;
// 	readBinaryDataFromFile2Memory("../bug.dat", (void*&)m_buffer, bites);
// 	if (bites != 512 * 4)std::cout << "read bug.dat failed" << std::endl;
// 	cudaMemcpyAsync(vpfeature, m_buffer, 512 * 4, cudaMemcpyHostToDevice, s);
	return m_pfaceQuery->query(vpfeature, vcudaStream, m_recognizationConfidence);
	cudaMemcpyAsync(m_buffer, vpfeature, 512 * 4, cudaMemcpyDeviceToHost, s);
	cudaStreamSynchronize(s);
	int maxId = -1;
	double old = m_recognizationConfidence;
	double sum = 0;
	for (int i = 0; i < m_personCount; ++i)
	{
		sum = 0;
		const float* vp = m_buffer;
		const CPerson& p = m_pfeatrues->persons(i);
		for (int k = 0; k < 512; ++k)
		{
			sum += *vp++ * p.featrue(k);
		}
...
bool CImagesFeatures::isImagesPath()
{
	if (m_videoOrImgPath.size() == 1 && m_videoOrImgPath.back() >= '0' && m_videoOrImgPath.size() <= 9)
		return false;
	int offset = m_videoOrImgPath.find('.');
	if (offset < 0 || offset >= m_videoOrImgPath.size()|| m_videoOrImgPath.size()<5)
		return true;
	return m_videoOrImgPath.back() == '/';
}

const std::string CImagesFeatures::getImageFileName(int i)
{
	return m_videoOrImgPath + m_imgNames[i];
}

const std::string CImagesFeatures::getPersonName(int vfaceId)
{
	if (vfaceId < 0 || vfaceId >= m_personCount)
		return "Person";
	std::string name  = m_pfeatrues->persons(vfaceId).name();
	if (m_pinyin)
...
void GetOtherCodePinYin(int nCode, std::string& strValue);
...
void GetPinYin(unsigned char* Chinese, std::string& PinYin)
{
	...
	for (int i = 0; i < chinese_str_len; i++)
	{
		...
		else
		{
			...
			for (int j = 396; j >= 0; j--) //��λ��
			{
				if (ascii_code >= code_qw[j])
				{
					std::string tmp = pinyin[j];
					tmp[0] -= 32;
					PinYin += tmp;
					break;
				}
