#include "Common.h"
#include "hf_ssdDetector.h"
#include "sharedBuffer.h"
#include "cropAndResize.h"
#include "postDetections.h"

CFactory<CSSDEngine> g_ssdCreator("ssd_hf");


CSSDEngine::CSSDEngine()
{
	std::string detectionModelPrefix = m_pconfiger->readValue("hf_detectionModelPrefix");
	_setBatchSize(m_pconfiger->readValue<int>("hf_detectionBatchSize"));
...
void CSSDEngine::postProcessV(std::vector<CDataShared*>& vmodelInputes, CDataShared* voutput)
{
	...
	if (para.area < 101)
	{
		...
		splitStringAndConvert(pconf->readValue("hf_scoreAreaAspect"), scoreAreaAspect, ',');
