...
__global__ void calculateSSDnumKernel(float vConfidence, float* viopSSD)
{
	extern __shared__ float sConsIndex[];
	float confidence = viopSSD[threadIdx.x*7+2];
	sConsIndex[threadIdx.x] = confidence;
	__syncthreads();
	if (confidence >= vConfidence)
	{
		float confidence2 = sConsIndex[1+threadIdx.x];
		if (vConfidence > confidence2)
			*viopSSD = threadIdx.x + 1.1f;
	}
}

void calculateSSDnum(float vConfidence, float* viopSSD, int vMaxDetetionNum, cudaStream_t stream)
{
	calculateSSDnumKernel << <1, vMaxDetetionNum, vMaxDetetionNum*sizeof(float), stream >> > (vConfidence, viopSSD);
}