...
inline __device__ int getOffset(int x, int y, int width)
{
	return y * width + x;
}

__global__ void ImageRotate_kernel_tex(unsigned char* tex, float3 *poDataOut, int nWidth, int nHeight,
...
//             readPixel2(BRPixel, x_1byte + 3.0f, y_1byte + 1.0f);
			memcpy(&TLPixel, tex + getOffset(x_1byte, y_1byte, nWidth * 3), 3);
			memcpy(&TRPixel, tex + getOffset(x_1byte+3, y_1byte, nWidth * 3), 3);
			memcpy(&BLPixel, tex + getOffset(x_1byte, y_1byte+1, nWidth * 3), 3);
			memcpy(&BRPixel, tex + getOffset(x_1byte+3, y_1byte+1, nWidth * 3), 3);
...
void Affine_::affineInterface(int vBatchSize, float *vp68pointsOnGPU,
                              unsigned char* vSrcImg, int vWidthSrcImg, int vHeightSrcImg,
                              float *vopDst) {
//    std::cout << "batch:" << vBatchSize << std::endl;
    ...
    if (preprocess == xxx) {
        ...
        exitIfCudaError(cudaMemcpyAsync(pts68ss.data(), vp68pointsOnGPU, ptsSize, cudaMemcpyDeviceToHost, stream));
        ...
        middle1 = clock();
        exitIfCudaError(cudaMemcpyAsync(points5.data(), dev_5kpts, num_pts * 10 * sizeof(float), cudaMemcpyDeviceToHost, stream));
        cudaStreamSynchronize(stream);
