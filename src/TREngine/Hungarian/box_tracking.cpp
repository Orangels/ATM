...
bool BoxTracker::tracking_Frame_Hungarian(vector<int>& votrackIds, vector<Rect>& detection_rects, int img_w_, int img_h_)
{    
    img_w = img_w_;
    img_h = img_h_;
    ...
    if (frames_num==0) {
        for (int j = 0; j < detection_rects.size(); j++){ 
            ...
            votrackIds.push_back(current_max_id);
            current_max_id +=1;
        }
    }
    else{
        ...
        for(int j=0;j<match_rectinfos.size();j++)
            votrackIds.push_back(match_rectinfos[j].obj_id);

         votrackIds.push_back(tracking_end_id.size());
         votrackIds.insert(votrackIds.end(),tracking_end_id.begin(),tracking_end_id.end());
    }
