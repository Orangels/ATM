...
namespace instant {
    ...
    inline int calc_total_size(std::vector<int> const& dims) {
    ...
    inline int total_size(array const& a) { return calc_total_size(a.dims()); }
    ...
    template <typename T>
    inline array uniforms(dtype_t d, std::vector<int> const& dims, T val)
	{
    ...
    inline array zeros(dtype_t d, std::vector<int> const& dims) {
